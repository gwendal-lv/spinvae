...
class Preset2dHelper:
    def __init__(self, ds: PresetDataset):
        ...
        unique_types = list(dict.fromkeys(self._param_types_str))  # dict is insertion-ordered (Python 3.7+)
        unique_types_indexes = {t: i for i, t in enumerate(unique_types)}
        self._param_types_tensor = torch.tensor(
        ...
        self.matrix_numerical_bool_mask = torch.tensor(
            [i in self._matrix_numerical_rows for i in range(self.n_learnable_params)], dtype=torch.bool
        )
        self.matrix_numerical_mask = self.matrix_numerical_bool_mask.float()
        self.matrix_categorical_bool_mask = torch.tensor(
            [i in self._matrix_categorical_rows for i in range(self.n_learnable_params)], dtype=torch.bool
        )
        ...
        self._pre_filled_matrix[:, 2] = self._param_types_tensor  # Set param types (will never change)
    ...
    def n_learnable_numerical_params(self):
        return len(self._matrix_numerical_rows)

    @property
    def n_learnable_categorical_params(self):
        return len(self._matrix_categorical_rows)
